...
/// Evaluation time inside the fixture WRPRC `iat`/`exp` period `[100, 200)`.
const FIXTURE_EVALUATION_SECONDS: i64 = 150;

fn evaluation_time() -> time::OffsetDateTime {
    at(FIXTURE_EVALUATION_SECONDS)
}
...
fn sign_wrprc_jades_profile(
    ...
    let thumbprint = reallyme_codec::base64url::bytes_to_base64url(
        ArtifactDigest::sha256(certificate_der).as_bytes(),
    );
    let header = format!(
        r#"{{"alg":"{algorithm_name}","typ":"{type_value}","iat":100,"x5c":["{certificate}"],"x5t#S256":"{thumbprint}"}}"#
    );
...
/// Registered CWT claim keys (RFC 8392 section 3.1 and the Token Status List
/// `status` claim) used to transcode the JSON fixture into a CWT claims set.
fn registered_cwt_key(name: &str) -> Option<usize> {
    match name {
        "iss" => Some(1),
        "sub" => Some(2),
        "aud" => Some(3),
...
fn wrprc_cwt_claims(json: &[u8]) -> Result<Vec<u8>, RegistrationError> {
    ...
    append_cbor_major_length(&mut output, 5, claims.len())?;
    for (name, value) in &claims {
        match registered_cwt_key(name) {
            Some(key) => append_cbor_major_length(&mut output, 0, key)?,
            None => append_cbor_text(&mut output, name)?,
        }
        append_cbor_value(&mut output, value)?;
    }
    Ok(output)
}

fn append_cbor_value(
    output: &mut Vec<u8>,
    ...
    match value {
        ...
        serde_json::Value::Number(number) => {
            ...
            append_cbor_major_length(output, 0, unsigned)?;
        }
        serde_json::Value::String(text) => append_cbor_text(output, text)?,
        serde_json::Value::Array(items) => {
            append_cbor_major_length(output, 4, items.len())?;
            for item in items {
                append_cbor_value(output, item)?;
            }
        }
        serde_json::Value::Object(members) => {
            append_cbor_major_length(output, 5, members.len())?;
            for (name, member) in members {
                append_cbor_text(output, name)?;
                append_cbor_value(output, member)?;
            }
        }
    }
    Ok(())
...
fn wrprc_receipt_binds_authenticated_representation_and_local_issuance_state(
    ...
    let cose_sign1 = sign_wrprc_cose(&cwt, &signer_certificate, &private_key)?;
    let cose_result =
    ...
    let ed25519_cose = sign_wrprc_cose_ed25519(&cwt, &ed25519_certificate, &ed25519_private_key)?;
    let ed25519_result =
        authenticate_wrprc_cose_sign1(RegistrationCertificateCoseAuthenticationInput {
