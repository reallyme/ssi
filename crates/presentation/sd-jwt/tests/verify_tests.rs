...
fn sd_jwt_vp_fails_if_claim_value_tampered() {
    ...
    let err = verify_sd_jwt_vp(&vp, &issued.envelope, &issuer_jwk, &issuer_pub, None).unwrap_err();
...
fn sd_jwt_vp_rejects_unbounded_or_inconsistent_merkle_disclosures() {
    ...
    let limit_error =
        verify_sd_jwt_vp(&valid, &issued.envelope, &issuer_jwk, &issuer_pub, None).unwrap_err();
    assert!(matches!(limit_error, SdJwtVpError::InvalidBundle));
}
