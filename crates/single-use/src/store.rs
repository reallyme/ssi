...
pub trait SingleUseStore: Send + Sync {
    ...
    fn put(&self, key: &str, expires_at_unix: u64) -> SingleUseResult<()>;
...
impl SingleUseState {
    fn schedule_expiration(&mut self, key: &str, expires_at_unix: u64) {
        self.expirations
    ...
    fn unschedule_expiration(&mut self, key: &str, expires_at_unix: u64) {
        let remove_bucket = if let Some(keys) = self.expirations.get_mut(&expires_at_unix) {
            if let Some(mut owned_key) = keys.take(key) {
                owned_key.zeroize();
            }
            keys.is_empty()
        } else {
    ...
    fn prune(&mut self, now_unix: u64) {
        while let Some((&expires_at_unix, _)) = self.expirations.first_key_value() {
            ...
            while let Some(mut key) = keys.pop_first() {
                if self.entries.get(&key).copied() == Some(expires_at_unix) {
                    if let Some((mut owned_key, _)) = self.entries.remove_entry(&key) {
                        owned_key.zeroize();
                    }
                }
                key.zeroize();
...
impl Drop for InMemorySingleUseStore {
    fn drop(&mut self) {
        ...
        for (mut key, _) in entries.entries.drain() {
            key.zeroize();
        }
...
impl SingleUseStore for InMemorySingleUseStore {
    fn put(&self, key: &str, expires_at_unix: u64) -> SingleUseResult<()> {
        ...
        if let Some(previous_expiry) = state.entries.get_mut(key) {
            if *previous_expiry == expires_at_unix {
                return Ok(());
            }
            let replaced_expiry = *previous_expiry;
            *previous_expiry = expires_at_unix;
            state.unschedule_expiration(key, replaced_expiry);
            state.schedule_expiration(key, expires_at_unix);
            return Ok(());
        }
        if state.entries.len() >= self.max_entries {
            return Err(SingleUseError::CapacityExceeded);
        }
        state.entries.insert(key.to_owned(), expires_at_unix);
        state.schedule_expiration(key, expires_at_unix);
        Ok(())
    }

    fn record_once(&self, key: &str, now_unix: u64, expires_at_unix: u64) -> SingleUseResult<()> {
        if key.is_empty() || key.len() > MAX_SINGLE_USE_KEY_BYTES || expires_at_unix <= now_unix {
            return Err(SingleUseError::InvalidKey);
        }
        let mut state = self.lock()?;
    ...
    fn consume(&self, key: &str, now_unix: u64) -> SingleUseResult<()> {
        ...
        match state.entries.remove_entry(key) {
            Some((mut owned_key, expires_at)) if expires_at > now_unix => {
                state.unschedule_expiration(&owned_key, expires_at);
                owned_key.zeroize();
                Ok(())
            }
            Some((mut owned_key, expires_at)) => {
                state.unschedule_expiration(&owned_key, expires_at);
                owned_key.zeroize();
